...
def get_train_test_dataset(language:str)->tuple:
    ...
    train_negative, test_negative = train_test_split(df[df["label"]==False], test_size=0.02, random_state=SEED)
    train_positive, test_positive = train_test_split(df[df["label"]==True], test_size=0.33, random_state=SEED)
...
def main():
    ...
    plot_results(test_answers, test_result, title=f"Predicted with Naive Bayes.\nIs comment about double quality?", save_path=f"results\\bayes_results_{lang}.png")
